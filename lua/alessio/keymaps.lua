...
vim.keymap.set(
  "n",
  "gb",
  funcs.go_back_and_close,
  vim.tbl_extend("force", opts, { desc = "Go back and close previous buffer if unchanged" })
)
...
vim.keymap.set(
  ...
  "<C-^>",
  [[<C-\><C-n>:b#<CR>]],
  vim.tbl_extend("force", opts, { silent = true, desc = "Switch to previous buffer" })
)
...
map("n", "<leader>fp", fzf.grep_project, { desc = "Grep project" })

-- Git-related commands
map("n", "<leader>fc", fzf.git_commits, opts)
map("n", "<leader>fb", fzf.git_bcommits, opts)
map("n", "<leader>fs", fzf.git_status, opts)
